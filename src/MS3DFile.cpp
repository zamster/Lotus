...
bool CMS3DFile::LoadFromFile(const char* lpszFileName)
{
	...
	for ( int k = 0 ; k < nNumMaterials ; k++ )
	{
		stbi_uc *buff;
		int comp, sizex, sizey;	
		char add[256] = "Data/";
		strcat( add, _i->arrMaterials[k].texture );
		buff = stbi_load( add, &sizex, &sizey, &comp, 0 );
		glGenTextures(1, &m_texture[k]);
		glBindTexture(GL_TEXTURE_2D, m_texture[k]);
...
void CMS3DFile::Draw()
{
	...
	{	
		int materialIndex = _i -> arrGroups[i].materialIndex;
		if ( materialIndex >= 0 )
		{
			glMaterialfv( GL_FRONT, GL_AMBIENT,   _i->arrMaterials[materialIndex].ambient );
			glMaterialfv( GL_FRONT, GL_DIFFUSE,   _i->arrMaterials[materialIndex].diffuse );
			glMaterialfv( GL_FRONT, GL_SPECULAR,  _i->arrMaterials[materialIndex].specular );
			glMaterialfv( GL_FRONT, GL_EMISSION,  _i->arrMaterials[materialIndex].emissive );
			glMaterialf(  GL_FRONT, GL_SHININESS, _i->arrMaterials[materialIndex].shininess );
			if ( m_texture[materialIndex] > 0 )
			{
				glEnable( GL_TEXTURE_2D );
				glBindTexture( GL_TEXTURE_2D, m_texture[materialIndex] );
			}
			else
		...
		for ( int j = 0 ; j < ( _i -> arrGroups[i] ).numtriangles; j++ )
		{
			word triangleIndex = ( _i -> arrGroups[i] ).triangleIndices[j];
			const ms3d_triangle_t* pTri =  &_i->arrTriangles[triangleIndex];
			
			for ( int k = 0 ; k < 3 ; k++ )
			{
				word index = pTri -> vertexIndices[k];
